...
export const OutlineButton: FC<ComponentPropsWithRef<'button'> & ButtonProps> = ({
  color = 'primary',
  size = 'normal',
  formAdjacent = false,
  children,
  ...
  return (
    <StyledButton color={color} theme={theme} size={size} formAdjacent={formAdjacent} {...props}>
      {children}
    </StyledButton>
  );
};
...
const StyledButton = styled.button<{ theme: Theme; color: ButtonColor; size: ButtonSize; formAdjacent: boolean }>`
  ...
  padding: ${({ theme, formAdjacent }) =>
    formAdjacent ? `${theme.space(2)}px ${theme.space(6)}px` : `${theme.space(3)}px ${theme.space(6)}px`};
